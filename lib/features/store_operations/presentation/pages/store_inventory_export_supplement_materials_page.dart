...
import 'store_inventory_export_supplement_material_draft_page.dart';

class StoreInventoryExportSupplementMaterialsPage extends ConsumerWidget {
  final int storeId;
  final StoreInventoryExportSupplementMaterialDraftSeedData? seedData;

  const StoreInventoryExportSupplementMaterialsPage({
    super.key,
...
class _ReadyViewState extends ConsumerState<_ReadyView> {
  ...
  void _addMaterial(StoreInventoryExportSupplementMaterialMockItem material) {
    setState(() {
      _quantitiesByCode[material.code] = 1;
    });
  }
  ...
  void _showDraft() {
    context.goNamed(
      RouteNames.storeInventoryExportSupplementMaterialDraft,
  ...
  Widget build(BuildContext context) {
    ...
    // specific supplement material permission when backend PBAC is available.
        ...
        child: SizedBox.expand(
          child: Stack(
            children: [
              Column(
                children: [
                  _SupplementMaterialsHeader(storeId: widget.storeId),
                  const _SupplementMaterialFilters(),
                  Expanded(
                    child: materials.isEmpty
                        ? const _EmptyMaterialView()
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _SupplementMaterialTile(
                                material: material,
                                quantity: _quantitiesByCode[material.code] ?? 0,
                                onAdd: () => _addMaterial(material),
                                onIncrement: () => _incrementMaterial(material),
                                onDecrement: () => _decrementMaterial(material),
                              );
                            },
              ...
              if (hasSelection)
                _SupplementMaterialsBottomActionBar(
...
class _SupplementMaterialsHeader extends StatelessWidget {
  final int storeId;

  const _SupplementMaterialsHeader({required this.storeId});
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            onPressed: () =>
                _showComingSoon(context, 'Tìm kiếm nguyên vật liệu'),
          ),
          IconButton(
            tooltip: 'Quét mã',
            ...
            color: AppColors.textPrimary,
            onPressed: () =>
                _showComingSoon(context, 'Quét mã nguyên vật liệu'),
          ),
        ],
      ),
    );
...
class _SupplementMaterialFilters extends StatelessWidget {
  const _SupplementMaterialFilters();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: const [
          Flexible(
            child: _SupplementMaterialFilterChip(label: 'Nguyên vật liệu'),
          ),
          SizedBox(width: AppConstants.spacingSm),
          Flexible(
            child: _SupplementMaterialFilterChip(label: 'Nhóm nguyên vật liệu'),
          ),
        ],
      ),
    );
...
class _SupplementMaterialFilterChip extends StatelessWidget {
  final String label;

  const _SupplementMaterialFilterChip({required this.label});
...
class _SupplementMaterialTile extends StatelessWidget {
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${material.code}  |  Còn: ${material.stockQuantity} ${material.unit}',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
            ...
            quantity > 0
                ...
                : _AddMaterialButton(code: material.code, onPressed: onAdd),
          ],
        ),
      ),
    );
...
class _MaterialQuantityStepper extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      key: Key('inventory_export_supplement_material_stepper_$code'),
      height: 36,
      ...
      child: Row(
        ...
        children: [
          _QuantityIconButton(
            key: Key('inventory_export_supplement_material_decrement_$code'),
          ...
          SizedBox(
            ...
            child: Text(
              quantity.toString(),
              key: Key('inventory_export_supplement_material_quantity_$code'),
          ...
          _QuantityIconButton(
            key: Key('inventory_export_supplement_material_increment_$code'),
...
class _AddMaterialButton extends StatelessWidget {
  ...
  const _AddMaterialButton({required this.code, required this.onPressed});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton(
        key: Key('inventory_export_supplement_material_add_$code'),
        onPressed: onPressed,
...
class _SupplementMaterialsBottomActionBar extends StatelessWidget {
  final int selectedQuantity;
  final VoidCallback onContinue;

  const _SupplementMaterialsBottomActionBar({
  ...
  Widget build(BuildContext context) {
    return Positioned(
      ...
      child: Material(
        ...
        child: InkWell(
          key: const Key(
            'inventory_export_supplement_materials_continue_action',
          ),
          onTap: onContinue,
          borderRadius: BorderRadius.circular(AppTheme.radiusMd),
          child: Container(
...
class _EmptyMaterialView extends StatelessWidget {
  const _EmptyMaterialView();
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppConstants.spacingXs),
            const Text(
              'Danh sách nguyên vật liệu bổ sung sẽ hiển thị tại đây.',

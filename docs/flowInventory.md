...
FE có thể gửi enum dưới dạng số hoặc tên enum. Khuyến nghị gửi tên để payload dễ đọc.
...
| --- | --- | --- |
| Tạo món | POST /api/products | Giữ nguyên; có thể gửi recipes ngay lúc tạo món. Nếu chưa gửi, lưu recipe qua API mới sau khi có productId. |
| Sửa món | PUT /api/products/{id} | Chỉ sửa metadata, giá, category, variants. Không sửa recipe ở đây. |
...
| Nguyên liệu | PUT /api/ingredients/{id}/quantity | Không dùng ở UI mới. Dùng import, adjustment hoặc wastage để luôn có lịch sử kho. Endpoint cũ còn tương thích nhưng sẽ deprecate. |
